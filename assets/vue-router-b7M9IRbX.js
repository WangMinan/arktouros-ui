import { s as ot, d as ze, u as Q, aw as st, k as N, A as le, r as it, w as ct, ab as Ve, n as at, m as q, X as lt } from "./@vue-ggRZ67L8.js";
/*!
* vue-router v4.5.1
...
const F = () => {
}, T = Array.isArray, Ge = /#/g, ft = /&/g, ht = /\//g, dt = /=/g, pt = /\?/g, Ke = /\+/g, mt = /%5B/g, gt = /%5D/g, Ue = /%5E/g, vt = /%60/g, De = /%7B/g, yt = /%7C/g, We = /%7D/g, Rt = /%20/g;
function ge(e) {
...
function jt(e) {
  ...
  function a(i, f, h) {
    const c = e.indexOf("#"), l = c > -1 ? (n.host && document.querySelector("base") ? e : e.slice(c)) + i : Bt() + e + i;
    try {
...
function zt(e) {
  e = xt(e);
  const t = jt(e), n = Ht(e, t.state, t.location, t.replace);
  function r(a, d = true) {
  ...
  function i(f) {
    ...
    for (const l of e) {
      ...
      for (const u of l) if (u.type === 0) h += u.value;
        ...
        if (T(A) && !S) throw new Error(`Provided param "${w}" is an array but it is not repeatable (* or + modifiers)`);
...
function Xt(e, t) {
  ...
  function a(c, l, u) {
    ...
    if ("alias" in c) {
      ...
      for (const j of I) A.push(Me(P({}, S, { components: u ? u.record.components : S.components, path: j, aliasOf: u ? u.record : S })));
    }
    let E, k;
    for (const I of A) {
      const { path: j } = I;
...
function $e(e) {
  ...
  }), a = N(() => o.value > -1 && ln(n.params, r.value.params)), d = N(() => o.value > -1 && o.value === n.matched.length - 1 && Qe(n.params, r.value.params));
  function g(i = {}) {
    if (an(i)) {
...
const sn = ze({ name: "RouterLink", compatConfig: { MODE: 3 }, props: { to: { type: [String, Object], required: true }, replace: Boolean, activeClass: String, exactActiveClass: String, custom: Boolean, ariaCurrentValue: { type: String, default: "page" }, viewTransition: Boolean }, useLink: $e, setup(e, { slots: t }) {
  const n = lt($e(e)), { options: r } = q(ne), o = N(() => ({ [He(e.activeClass, r.linkActiveClass, "router-link-active")]: n.isActive, [He(e.exactActiveClass, r.linkExactActiveClass, "router-link-exact-active")]: n.isExactActive }));
  return () => {
...
function ln(e, t) {
  for (const n in t) {
    ...
    if (typeof r == "string") {
      if (r !== o) return false;
    } else if (!T(o) || o.length !== r.length || r.some((a, d) => a !== o[d])) return false;
  }
  return true;
}
function Be(e) {
...
const He = (e, t, n) => e ?? t ?? n, un = ze({ name: "RouterView", inheritAttrs: false, props: { name: { type: String, default: "default" }, route: Object }, compatConfig: { MODE: 3 }, setup(e, { attrs: t, slots: n }) {
  const r = q(me), o = N(() => e.route || r.value), a = q(Le, 0), d = N(() => {
    let f = Q(a);
  ...
  le(Le, N(() => d.value + 1)), le(rn, g), le(me, o);
  const i = it();
...
const fn = un;
function mn(e) {
  const t = Xt(e.routes, e), n = e.parseQuery || tn, r = e.stringifyQuery || Ne, o = e.history, a = W(), d = W(), g = W(), i = ot(B);
  let f = B;
  ...
  function A(s, m) {
    ...
    return P({ fullPath: O, hash: C, query: r === Ne ? nn(s.query) : s.query || {} }, v, { redirectedFrom: void 0, href: R });
  }
  function E(s) {
  ...
  function ve(s, m) {
    ...
    return p.push(R), z(p).then(() => {
      ...
      p = [];
      for (const y of O) if (y.beforeEnter) if (T(y.beforeEnter)) for (const _ of y.beforeEnter) p.push(H(_, s, m));
  ...
  function rt() {
    return J && i.value !== B ? Promise.resolve() : new Promise((s, m) => {
      oe.add([s, m]);
    });
  }
  function ie(s) {
  ...
  function we(s, m, p, v) {
    ...
    return at().then(() => C(s, m, O)).then((R) => R && Nt(R)).catch((R) => se(R, s, m));
  }
  const ce = (s) => o.go(s);
  let ae;
  const ee = /* @__PURE__ */ new Set(), Pe = { currentRoute: i, listening: true, addRoute: u, removeRoute: w, clearRoutes: t.clearRoutes, hasRoute: b, getRoutes: S, resolve: A, options: e, push: I, replace: j, go: ce, back: () => ce(-1), forward: () => ce(1), beforeEach: a.add, beforeResolve: d.add, afterEach: g.add, onError: Ee.add, isReady: rt, install(s) {
    const m = this;
    s.component("RouterLink", cn), s.component("RouterView", fn), s.config.globalProperties.$router = m, Object.defineProperty(s.config.globalProperties, "$route", { enumerable: true, get: () => Q(i) }), V && !ae && i.value === B && (ae = true, I(o.location).catch((C) => {
    }));
    const p = {};
    for (const C in B) Object.defineProperty(p, C, { get: () => i.value[C], enumerable: true });
    s.provide(ne, m), s.provide(et, st(p)), s.provide(me, i);
    const v = s.unmount;
    ee.add(s), s.unmount = function() {
      ee.delete(s), ee.size < 1 && (f = B, U && U(), U = null, i.value = B, ae = false, J = false), v();
    };
  } };
  function z(s) {
